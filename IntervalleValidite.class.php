    ...
    function __construct(array $intervalles=null) {
        ...
        foreach($this->intervalles as &$intervalle) {
            if (is_array($intervalle)) {
                $intervalle['debut']=str_replace([' ', '+'], '', $intervalle['debut']);
                $intervalle['fin']=str_replace([' ', '+'], '', $intervalle['fin']);
                if (array_key_exists('sauf', $intervalle)) {
                    $intervalle['sauf'] = str_replace([' ', '+'], '', $intervalle['sauf']);
                }
            }
            else {
                $intervalle = str_replace([' ', '+'], '', $intervalle);
            }
        }
    }
