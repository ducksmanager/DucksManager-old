<?php
class SPGdev extends Edge {
	function SPGdev($numero) {
		$this->numero=$numero;
	}
	function dessiner() {
		if ($this->numero<=57) {
			$this->largeur=20*Edge::$grossissement;
			$this->hauteur=219.7*Edge::$grossissement;
		...
		elseif ($this->numero<=88) {
			...
			imagefilledrectangle($image, 0, 0, $this->largeur, $this->hauteur, $noir);
			imagefilledrectangle($image, .25*Edge::$grossissement, .25*Edge::$grossissement, $this->largeur-.25*Edge::$grossissement, $this->hauteur-.25*Edge::$grossissement, $fond);
			$noir = imagecolorallocate($image, 0, 0, 0);
			$blanc = imagecolorallocate($image, 255,255,255);
			...
			if ($this->numero<100)
				imagecopyresampled ($image, $icone, 0, $this->hauteur-2.1*$this->largeur-$nouvelle_hauteur/2, 0, 0, $this->largeur, $nouvelle_hauteur, $width, $height);
			else
				imagecopyresampled ($image, $icone, 0, $this->hauteur-3.5*$this->largeur-$nouvelle_hauteur/2, 0, 0, $this->largeur, $nouvelle_hauteur, $width, $height);
