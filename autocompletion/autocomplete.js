...
var fichier_options=null;
var num_div=null;
function initAutoComplete(form,field,submit,fic_options,numero_div){
  fichier_options=fic_options;
  num_div=numero_div;
  _documentForm=form;
  _inputField=field;
...
function callSuggestions(valeur){
	var _adresseRecherche = "autocompletion/"+fichier_options //l'adresse à interroger pour trouver les suggestions
   
	var arguments="?debut="+valeur;
    if (fichier_options=='options2.php')
    	arguments+='&pays='+pays_sel;
  	var myAjax = new Ajax.Request(_adresseRecherche, {
		   method: 'GET',
		   parameters:arguments,
		   onSuccess:function(transport,json) {
		    	var liste = traiteXmlSuggestions(transport.responseXML)
...
function creeAutocompletionDiv() {
  ...
  _completeDiv.id="completeDiv"+num_div;
...
var divOnMouseDown=function(){
  _inputField.value=getSuggestion(this);
  	$('completeDiv'+num_div).style.display='none';
  if (num_div==1) {
  	pays_sel=_inputField.value;
  	pays_sel=pays_sel.substring(pays_sel.lastIndexOf('(')+1,pays_sel.lastIndexOf(')'))
  	_resultCache=new Object();
  	initAutoComplete($('form-test2'),$('champ-texte2'),$('bouton-submit2'),'options2.php',2);
  	return;
  }
  else
  	magazine_sel=_inputField.value;
  	magazine_sel=magazine_sel.substring(magazine_sel.lastIndexOf('(')+1,magazine_sel.lastIndexOf(')'))
};
