...
(function($){
	$.fn.scrollbarTable = function(i){
		...
		return this.each(function(){
			...
			var cols = [];
			$t.find('tr:nth-child(-n+2) th,tr:nth-child(-n+2) td').each(function(){
				cols.push($(this).width());
			});
			
			/* Remove the header and put it before the rest of the table as a new table*/
			var $firstRow = $t.clone();
			$firstRow.find('tr:not(:nth-child(-n+2))').remove();
			$t.find('tr:nth-child(-n+2)').remove();
			$t.before($firstRow);
			
			/* Make the header, and the scrolling table, column widths the same as the original */
			$firstRow.find('tr:nth-child(-n+2) th,tr:nth-child(-n+2) td').each(function(i){
				$(this).attr('width',cols[i]);
			});
			$t.find('tr:nth-child(-n+2) th,tr:nth-child(-n+2) td').each(function(i){
				$(this).attr('width',cols[i]);
			});
