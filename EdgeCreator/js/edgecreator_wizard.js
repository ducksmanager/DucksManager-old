...
function wizard_init(wizard_id) {
	switch(wizard_id) {
		...
		case 'wizard-proposition-clonage':
			...
			$.each($('#'+wizard_id+' [name="wizard_numero"] option.tranche_prete'),function() {
				...
				if (index_numero_courant > index_numero_selectionne) {
					if (!nouvelle_tranche_placee) {
						if (tranches_pretes.length > NB_MAX_TRANCHES_SIMILAIRES_PROPOSEES) // Filtre sur les 5 derni貥s pr飩dentes
			...
			if (!nouvelle_tranche_placee) {
				// Entrer ici signifie qu'il n'y a pas de tranches pr괥s apr賠le num鲯 s鬥ctionn鍊				if (tranches_pretes.length > NB_MAX_TRANCHES_SIMILAIRES_PROPOSEES) // Filtre sur les 5 derni貥s pr飩dentes
					tranches_pretes=tranches_pretes.slice(tranches_pretes.length-NB_MAX_TRANCHES_SIMILAIRES_PROPOSEES, tranches_pretes.length);
				tranches_pretes.push(numero_selectionne);
			}
				...
				success:function(data) {
					...
					$.ajax({ // D鴡ils des 鴡pes
						...
						success:function(data) {
							...
							chargements.push('all'); // On ajoute l'鴡pe finale
...
function placer_dialogues_preview() {
	...
	dialogues.sort(function(dialogue1,dialogue2) { // Tri鳠par offset gauche, de droite ࠧauche
	...
	// Positionnement de la zone d'ajout d'鴡pe
					...
					if (section_active_positionnement) {
						...
						tester(function() {
							load_myfonts_preview(false,false,true,function() {
								...
								if (valeurs['Mesure_depuis_haut'] == 'Non') { // Le pos_y est mesur頥ntre le haut de la tranche et le bas du texte
...
function tester(callback, modif_dimensions) {
	...
	chargement_courant=0;
	var parametrage=form_options.serialize();
    charger_preview_etape(chargements[0],true,parametrage,function() { // Test de l'鴡pe finale
