...
<head>
    ...
    <script type="text/javascript">
        ...
        function reload_preview(val_numero,est_visu) {
            if (typeof(val_numero) == 'undefined') {
                reload_preview($('preview_issue').options[$('preview_issue').selectedIndex].value,true);
                return;
            }
            ...
            charger_image($('preview_prefix').readAttribute('src')+'/'+val_numero+'/'+val_zoom+'/'+etapes_preview.join("-")+'/'+JSON.stringify(parametrage)+'/'+(est_visu?'false':'true'),val_zoom,est_visu);
        }
        
        function charger_image(src,val_zoom,est_visu) {
            ...
            image.observe('load',function() {
                if (est_visu) {
                    ...
                    if (numeros_gen[numero_gen_courant])
                        reload_preview(numeros_gen[numero_gen_courant],false);
                }
        ...
        function reload_gen() {
            ...
            reload_preview(numeros_gen[0],false);
        }
        ...
        function par_defaut(nom_option) {
            ...
            $$('[name="numero_debut"]').each(function(debut_intervalle_possible) {
        ...
        function parametersArray(parametres) {
            return new Array(parametres.ordre,
                             parametres.fonction,
                             getValeur($$('[name="numero_debut"]')),
                             getValeur($$('[name="numero_fin"]')));
        }
