...
if (!is_null($user)) {
	...
	$identifiants_valides=count(DM_Core::$d->requete_select_distante($requete_identifiants_valides)) == 1;
	if ($identifiants_valides) {
		$requete_permission='SELECT 1 FROM edgecreator_droits WHERE username=\''.$user.'\' AND privilege=\'Admin\'';
		$permission_valide=count(DM_Core::$d->requete_select_distante($requete_permission)) == 1;
