...
function initProgressBar(id,url, parameters, callback) {
    ...
    jQuery.post(url, jQuery.extend({}, parameters || {}, {init_chargement: 'true', id: id}), function(response) {
        chargements[id]=response;
        jQuery.each(chargements[id], function(i, chargement) {
...
function getPctCourant(id_chargement) {
    return parseInt(100*((getIndexCourant(id_chargement)+1)/(chargements[id_chargement].length)));
}
